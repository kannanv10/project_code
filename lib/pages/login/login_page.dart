...
import 'package:node_auth/pages/login/login_state.dart';
...
class _LoginPageState extends State<LoginPage>
  ...
  void initState() {
    ...
    context.bloc<LoginBloc>().message$
        .exhaustMap(handleMessage)
        .collect()
        .disposedBy(bag);

    context.bloc<LoginBloc>().isLoading$
        .listen((isLoading) {
      if (isLoading) {
  ...
  Stream<void> handleMessage(message) async* {
    if (message is LoginSuccessMessage) {
      ...
      // Ignore the snackbar in context during navigation
      context.hideCurrentSnackBar();

      final email = emailController.text.replaceAll('@', '_').replaceAll('.', '');
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('userEmail', email);

      // Navigate to GreenHouseDetailsPage
      await Navigator.of(context).pushReplacementNamed(
        GreenHouseDetailsPage.routeName,
        arguments: email,
      );
      return;
    }
...
//       context.hideCurrentSnackBar();
//       // ignore: use_build_context_synchronously
//       await Navigator.of(context).pushReplacementNamed(GreenHouseDetailsPage.routeName);
//       return;
//     }
//     if (message is LoginErrorMessage) {
//       context.showSnackBar(message.message);
